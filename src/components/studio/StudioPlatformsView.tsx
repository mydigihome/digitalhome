...
export default function StudioPlatformsView() {
  ...
  return (
    <div className="space-y-5 max-w-[1200px] mx-auto">
      {/* Platform Selector */}
      <div className="flex items-center gap-0 overflow-x-auto" style={{ borderBottom: "1px solid #e5e7eb" }}>
        {PLATFORMS.map(p => (
          <button
            key={p.id}
            onClick={() => setActivePlatform(p.id)}
            className={`flex items-center gap-2 px-4 py-2.5 text-[13px] whitespace-nowrap transition border-b-2 ${
              activePlatform === p.id
        ...
        <button className="px-4 py-2.5 text-[13px] text-[#9ca3af] whitespace-nowrap hover:text-[#374151] transition border-b-2 border-transparent">
          + Connect
        </button>
      </div>
      ...
      <div className="bg-white dark:bg-[#1e2130] p-4" style={{ border: "1px solid #e5e7eb" }}>
        ...
        <div className="w-full h-1 bg-[#f0f0f0] dark:bg-[#252836]">
          <div className="h-full bg-[#111827] dark:bg-[#f9fafb]" style={{ width: `${progress}%` }} />
        </div>
        <div className="flex items-center gap-6 mt-2">
          <span className="text-[13px] font-medium text-[#111827] dark:text-[#f9fafb]">{(100000 - platform.followers).toLocaleString()} followers to go</span>
          <div className="flex items-center gap-4 text-[10px] text-[#9ca3af]">
            <span><span className="uppercase tracking-wider">YTD Growth:</span> <span className="text-[13px] font-medium text-[#111827] dark:text-[#f9fafb]">+{platform.growth30d * 3}</span></span>
            <span><span className="uppercase tracking-wider">Monthly Pace:</span> <span className="text-[13px] font-medium text-[#111827] dark:text-[#f9fafb]">+{platform.growth30d}/month</span></span>
            <span><span className="uppercase tracking-wider">ETA:</span> <span className="text-[13px] font-medium text-[#111827] dark:text-[#f9fafb]">{Math.ceil(eta / 30)} years</span></span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-[#1e2130] p-4" style={{ border: "1px solid #e5e7eb" }}>
        <div className="flex items-center justify-between mb-3">
          ...
          <button className="flex items-center gap-1 text-[11px] text-[#9ca3af] hover:text-[#374151] transition">
          ...
          <AreaChart data={GROWTH_DATA}>
            <defs>
              <linearGradient id="platformGrad" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor={platform.color} stopOpacity={0.1} />
      ...
      <div className="bg-white dark:bg-[#1e2130] p-4" style={{ border: "1px solid #e5e7eb" }}>
        ...
        <div className="space-y-0">
          {MOCK_POSTS.map((post, i) => (
            <div key={post.id} className="flex gap-3 py-3" style={{ borderBottom: i < MOCK_POSTS.length - 1 ? "1px solid #f5f5f5" : "none" }}>
      ...
      <div className="bg-white dark:bg-[#1e2130] p-4" style={{ border: "1px solid #e5e7eb" }}>
        ...
        <button className="text-[11px] text-[#6366f1] font-medium mt-3">Refresh Analysis</button>
      </div>
    </div>
  );
}
