  ...
  return (
    <div className={`bg-card border border-border rounded-xl shadow-[0_1px_3px_rgba(0,0,0,0.06)] p-5 ${expanded ? "fixed inset-4 z-50 overflow-auto" : ""}`}>
      ...
      <div className="flex items-start justify-between mb-1">
        <div>
          <h3 className="text-lg font-bold text-foreground">Total Earnings</h3>
          <p className="text-[13px] text-muted-foreground">Overview of your monthly earnings at a glance.</p>
        </div>
      ...
      <div className="flex items-end gap-3 mb-4 mt-2">
        ...
        <span className={`text-xs font-semibold px-2 py-0.5 rounded-full mb-1 ${pctChange >= 0 ? "bg-success/10 text-success" : "bg-destructive/10 text-destructive"}`}>
      ...
      <div style={{ height: expanded ? 360 : 180 }}>
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data}>
            ...
            />
            <Area type="monotone" dataKey="amount" stroke="#7B5EA7" strokeWidth={2} fill="url(#earningsFill)" />
          </AreaChart>
        </ResponsiveContainer>
      </div>
    </div>
