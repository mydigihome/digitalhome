  ...
  const processFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      if (file.type === "text/csv" || file.name.endsWith(".csv")) {
        ...
        text = await file.text();
        // If the text is mostly binary, try ArrayBuffer approach
        if (text.length < 100 || /[\x00-\x08]/.test(text.slice(0, 100))) {
          // Send as base64 for the AI to interpret
      ...
      if (txns.length === 0) {
        toast.error("No transactions found in the file. Try a CSV or text-based PDF.");
        setUploading(false);
        return;
      }

      // Determine month from first transaction or use current
  ...
  return (
    <div className="space-y-6">
      ...
      >
        {uploading ? (
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-sm text-muted-foreground">Analyzing your statement with AI...</p>
          </div>
        ) : (
          <label className="cursor-pointer flex flex-col items-center gap-3">
            <Upload className="h-8 w-8 text-muted-foreground" />
            <div>
              <p className="text-sm font-medium text-foreground">Upload your bank statement (PDF or CSV)</p>
              <p className="text-xs text-muted-foreground mt-1">Drag & drop or click to browse • Supports Chase, BofA, Wells Fargo, and more</p>
            </div>
      ...
      <div className="flex items-center gap-2 text-xs text-muted-foreground bg-muted/30 rounded-lg px-4 py-2.5">
        ...
        Your statement is processed securely and never stored or shared. Only the extracted transaction data is saved locally.
      </div>

      {/* Month selector */}
      {availableMonths.length > 0 && (
        <div className="flex items-center gap-3">
          <span className="text-sm font-medium text-foreground">Viewing:</span>
          <Select value={selectedMonth} onValueChange={setSelectedMonth}>
            <SelectTrigger className="w-44"><SelectValue /></SelectTrigger>
            <SelectContent>
              {availableMonths.map(m => (
                <SelectItem key={m} value={m}>{new Date(m + "-01").toLocaleDateString("en-US", { month: "long", year: "numeric" })}</SelectItem>
              ))}
            </SelectContent>
          </Select>
        </div>
        ...
        <>
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="rounded-xl border border-border bg-card p-5">
              ...
              <p className={`text-2xl font-bold ${totalIncome - totalSpent >= 0 ? "text-emerald-600" : "text-red-600"}`}>
          ...
          {pieData.length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {/* Transactions table */}
              <div className="rounded-xl border border-border bg-card p-5">
                <h3 className="text-sm font-semibold text-foreground mb-4">Transactions ({transactions.length})</h3>
                <div className="max-h-[300px] overflow-y-auto space-y-1">
                  {transactions.map((t, i) => (
                    <div key={i} className="flex items-center gap-3 py-2 px-2 rounded-lg hover:bg-muted/30 transition-colors text-sm">
      ...
      {yearlyData.length > 1 && (
        <div className="rounded-xl border border-border bg-card p-5">
          <h3 className="text-sm font-semibold text-foreground mb-4">Yearly Overview</h3>
      ...
      {transactions.length === 0 && !uploading && (
        <div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <h3 className="text-lg font-semibold text-foreground mb-1">No spending data yet</h3>
          <p className="text-sm text-muted-foreground max-w-sm">Upload a bank statement to automatically categorize your spending and track month-over-month trends.</p>
        </div>
      )}
    </div>
  );
